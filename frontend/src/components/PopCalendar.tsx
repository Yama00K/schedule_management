...
function CustomDay(props: DayProps, eventdata: EventData[]) {
  ...
  const isToday =
    format(new Date(), 'yyyy-MM-dd') === format(props.day.date, 'yyyy-MM-dd');
              ...
              {dayData.events.map((event, index) => (
                ...
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-gray-600">{event.contents}</p>
...
export default function PopCalendar() {
  ...
  useEffect(() => {
    const loadEvents = async () => {
      ...
      try {
        // ✅ ページめくり検知ログ
        console.log(
          `📅 カレンダーページ変更: ${date.getFullYear()}年${date.getMonth() + 1}月`
        );
        console.log(
          `📅 依存配列トリガー: year=${date.getFullYear()}, month=${date.getMonth()}`
        );
        ...
        const finalEvents: EventData[] = Array.from(eventsMap.entries()).map(
          ([date, events]) => ({
            date,
            events,
          })
        ...
        console.log(
          `✅ ページめくり完了: ${finalEvents.length}日分のデータ（総イベント数: ${finalEvents.reduce((sum, day) => sum + day.events.length, 0)}件）`
        );
      } catch (error) {
  ...
  const CustomDayWithEvents = useCallback(
    (props: DayProps) => {
      return CustomDay(props, eventdata);
    },
    [eventdata]
  ); // eventdataが変わったときだけ、この関数を再生成する
