...
export default function Home() {
    const [schedules, setSchedules] = useState<AppSchedule[]|undefined>(undefined);
    ...
    const handleFilterChange = async (filters: { tag?: string; year?: number; month?: number; day?: number }) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/schedules?${params.toString()}`, {
                method: 'GET',
    ...
    // 検索条件を親コンポーネントに渡す関数
    const handleSearch = (params: {tag?: string; year?: number; month?: number; day?: number }) => {
        setTag(params.tag || "");
        ...
        <div>
            <div className="container mx-auto p-4">
                            ...
                            {schedules.map((schedule) => (
                                <AccordionItem key={schedule.id} value={`item-${schedule.id}`}>
                                    ...
                                    <AccordionContent>
                                        ...
                                        <p>開始日時: {schedule.start_time.toLocaleString(undefined, customOptions)}</p>
                                        <p>終了日時: {schedule.end_time.toLocaleString(undefined, customOptions)}</p>
                                    </AccordionContent>
                                    {deleteMode && (
                                        <div className="flex justify-end">
                                            <Button variant="destructive" onClick={() => handleDelete(schedule.id)}>削除</Button>
                                        </div>
                                    )}
                                </AccordionItem>
                            ))}
                ...
                <div className="mt-6 space-x-4">
                    <Button variant="default" onClick={() => setDeleteMode(!deleteMode)}>編集</Button>
                    <Link href="/">
